...
# --- NEW ADDED: 限制配准次数 ---
registration_count = 0  # 记录初始定位后成功的配准次数
MAX_REGISTRATION_COUNT = 5  # 最大配准次数
# --------------------------------
...
def thread_localization():
    global T_map_to_odom, registration_count
    ...
    while True:
        ...
        if registration_count < MAX_REGISTRATION_COUNT:
            if global_localization(T_map_to_odom):
                registration_count += 1
                rospy.loginfo(f"Registration count: {registration_count}/{MAX_REGISTRATION_COUNT}")
                ...
                if registration_count >= MAX_REGISTRATION_COUNT:
                    ...
                    rospy.logwarn("The system will now rely purely on odometry.")
                    rospy.logwarn("="*60)
        else:
            # 超过5次，不进行配准，T_map_to_odom 保持不变
            # 也不打印，避免刷屏
            pass
        # ----------------------------------------
...
if __name__ == '__main__':
    ...
    rospy.init_node('limited_global_localization')
    rospy.loginfo('Limited Global Localization Node Inited...')
    rospy.loginfo(f'Will perform maximum {MAX_REGISTRATION_COUNT} scan-to-map registrations.')
    rospy.loginfo('After that, the system will use pure odometry.')
