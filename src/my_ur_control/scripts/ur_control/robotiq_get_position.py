    ...
    def activate(self, auto_calibrate: bool = True):
        ...
        if not self.is_active():
            ...
            while (not self._get_var(self.ACT) == 1 or not self._get_var(self.STA) == 3):
                print(self._get_var(self.ACT), self._get_var(self.STA))
                time.sleep(0.01)
...
if __name__ == '__main__':
    ...
    gripper.connect('192.168.1.188', 63352)

    #f = open("/home/zzy/pycharm/projects/ur_slip_control/grasp_slip_net/data/online/gripper_position.txt", 'a')
    gripper.activate()
    position = int(gripper.get_current_position() / 255 * 100)
    print("current gripper position:", position)
    #f.write(str(time.time()) + " " + str(position) + '\n')
    #f.close()