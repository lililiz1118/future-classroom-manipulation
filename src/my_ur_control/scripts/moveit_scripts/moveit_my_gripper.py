...
def get_transform(target_frame, source_frame):
    ...
    try:
        ...
        trans = tf_buffer.lookup_transform(target_frame, source_frame, rospy.Time(), rospy.Duration(1.0))
...
def MoveItIkControl(reference_frame,  target_xyz, target_quaternion, execution = 1):
    # 初始化move_group的API
    ...
    # 适当选择 planner id（按你的 moveit config 选择），RRTConnect 是常见默认
    try:
        arm.set_planner_id("RRTConnectkConfigDefault")
    except Exception:
        rospy.logwarn("set_planner_id 失败，使用默认 planner")
...
if __name__ == "__main__":
    ...
    # 由抓姿模型给出的目标位置
    cam_tar_xyz = np.array([ 0.00658794,-0.04595708 , 0.4509999 ])
    R_mat = np.array(
        [[ 0.32332367, -0.9414929  , 0.09514648],
        [-0.3874498  ,-0.22344396 ,-0.89440227],
        [ 0.86333334 , 0.25231695 ,-0.437026  ]]
    ...
    R_final_matrix = R_mat @ Ry_local @ Rz_local

    r = R.from_matrix(R_final_matrix)
    cam_tar_q = r.as_quat()  # 注意：顺序为 [x, y, z, w]
    print(cam_tar_q)
    ...
    # 机械臂base下目标位置
    base_tar_xyz, base_tar_q = transform_target_to_base(effort_tar_xyz, effort_tar_q, "ur_arm_base_link", "ur_arm_wrist_3_link")
    print("base")
    print(base_tar_xyz)
    print(base_tar_q)

    # 末端执行器转夹爪位置
    gripper_xyz, gripper_q = compensate_gripper_offset(base_tar_xyz, base_tar_q, [0 ,0 ,0.12], [0, 0 ,0 , 1])
    # 夹爪姿态调整
    gripper_q_new= align_y_down_around_z(gripper_q)
    q = gripper_q_new

    # gripper_xyz = [round(x, 3) for x in gripper_xyz]
    # gripper_q_new = [round(x, 3) for x in gripper_q_new]
    
    print(gripper_xyz)
    gripper_q_new

    base_link_xyz, base_link_q = transform_target_to_base(gripper_xyz, gripper_q_new, "base_link", "ur_arm_base_link")

    # base_link_xyz = [round(x, 3) for x in base_link_xyz]
    # base_link_q = [round(x, 3) for x in base_link_q]


    # ur_arm__base_link y轴超前
    # base_link x轴超前
    # base_link_xyz[0] -= 0.3
    print(base_link_xyz)
    # MoveItIkControl('base_link', base_link_xyz, base_link_q, execution=0)
    MoveItIkControl('ur_arm_base_link', gripper_xyz, gripper_q_new, execution=0)
    print(gripper_xyz)
    print(gripper_q_new)
