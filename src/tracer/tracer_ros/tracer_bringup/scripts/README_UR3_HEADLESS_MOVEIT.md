...
该入口不使用示教器、External Control URCap、键盘或 MoveIt Servo。它在人工确认后通过 Dashboard 上电和松闸，随后由 UR ROS Driver 2.4.1 直接发送控制 URScript，并在控制链 Ready 后打开 RViz。

## 安全要求

- 工作区必须无人，独立硬件急停必须真实可用；SSH、Ctrl+C 和软件节点都不能替代急停。
...
启动器显示 `192.168.131.3` 的 robot/safety mode、标定哈希和目标速度。只有精确输入大写 `START` 才会调用 Dashboard。默认 speed slider 为 5%。
...
在启动终端按 Ctrl+C，启动器会按 RViz、move_group、UR Driver 的逆序停止进程。关闭 RViz 也会结束本次控制链。默认不自动给 UR3 断电。

如果启动器报告已有 `/servo_server`、`/keyboard_jog`、`/move_group` 或 `/ur/ur_hardware_interface`，先停止旧控制入口再重试；不要绕过并发检查。
